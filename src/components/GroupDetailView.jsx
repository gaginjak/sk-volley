      ...
      {activeTab === 'players' ? (
        <div style={{ display: 'grid', gap: 10 }}>
          ...
          {players.length === 0 ? <div style={{ color: '#94a3b8', fontSize: 13 }}>Nema igrača u ovoj grupi.</div> : players.map((player) => {
            const medicalStatus = getMedicalStatus(player.medical_expiry_date || player.medical)
            return (
              <div key={player.id} style={{ background: '#111827', borderRadius: 12, padding: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  {medicalStatus !== 'ok' ? <span style={{ color: medicalStatus === 'expired' ? '#fca5a5' : '#fde68a', fontSize: 12, fontWeight: 700 }}>{getMedicalLabel(medicalStatus)}</span> : null}
